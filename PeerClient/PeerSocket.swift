...
//        print("WebSocket closed. reason: \(String(describing: reason))")
        // Đoạn này comment lại để ko tự ngắt nếu socket server tèo, hoặc timeout
//        self.isDisconnected = true
//
...
//        self.delegate?.webSocket(webSocket: self, didCloseWithReason: reason)
    ...
    func myDisconnect()
    {
        ...
        // in case of socket is closed by server, delegate is needed to notify socket closed
        self.delegate?.webSocket(webSocket: self, didCloseWithReason: "Customer disconnect")
    }
